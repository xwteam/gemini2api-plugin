...
> [!IMPORTANT]
> Chrome 禁止扩展目录内出现 `_` 开头的文件/文件夹（**`_locales` 除外**）。v1.2.2 起开发用 `doc-templates/`、`gen_docs.py` 已去掉 `_` 前缀；若仍报 `_doc_templates` 相关错误，请拉取最新代码。
...
A：请更新到 **v1.2.2+** 并重新拉取仓库。旧版含 `_` 前缀的开发目录会导致 Chrome 拒绝加载。
...
|------|------|
| `README.md` | 简体中文主文档 |
| `doc-templates/` | 繁中 / 英 / 日 / 韩 文档模板 |
| `gen_docs.py` | 生成 `docs/{locale}/README.md` |
| `CHANGELOG.md` | 版本变更记录 |

更新文档：改 `README.md` 或 `doc-templates/*.md` 后执行 `python gen_docs.py` 重新生成五语 `docs/`。
