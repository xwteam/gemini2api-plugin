...
- **多账号兼容**：可指定账号 ID 精确更新，留空则处理所有过期账号
...
| **账号 ID** | 可选。多账号时建议填（如 `account-0`）；留空则处理所有过期账号 |
...
> [!IMPORTANT]
> 使用前提：浏览器里要**常开一个已登录的 `gemini.google.com` 标签页**，并与中转站使用同一个 Google 账号。插件刷新的就是它——这是新 Cookie 的来源。标签页或浏览器关闭后，过期将无法自动修复。
