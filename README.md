...
- **防串号（PSID 身份校验）**：提交前用本地 PSID 与目标账号比对，只把 Cookie 提交给身份一致的账号，多账号绝不张冠李戴
...
| **账号 ID** | 可选。填写后只校验该账号（如 `account-0`）；留空则在过期账号中自动匹配本浏览器登录的账号 |
...
> - 插件已内置 PSID 身份校验：本地账号与目标账号对不上时会**拒绝提交并告警**，但这只是最后一道防线，正确做法仍是一个浏览器专管一个账号。
...
- **弹窗面板**：显示各账号活动/过期状态、PSID 打码预览、最近动作日志；`立即检查` 手动跑一轮轮询，`强制刷新提交` 立即刷新并提交。
- **图标角标**：
  - 蓝色数字 = 活动账号数
  - 红色数字 = 过期账号数
  - `!` = 连接中转站失败
