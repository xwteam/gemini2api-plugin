...
| **API Key** | 中継の `sk-` プレフィックス付きキー |
| **アカウント ID** | **推奨**（例：`account-0`）：指定アカウントに直接送信。空欄時は PSID 自動マッチ |
| **ポーリング間隔** | 状態確認間隔、デフォルト 60 秒、最小 30 秒 |
| **更新クールダウン** | 更新後の再処理抑制、デフォルト 120 秒 |

**接続テスト** → **保存**（中継ドメインの権限を許可してください）。

> [!IMPORTANT]
> **1 ブラウザインスタンス = 1 アカウントのみ。** `gemini.google.com` の Cookie は 1 セットのみ。ログイン中の **gemini.google.com タブを常時開く**、**専用ブラウザプロファイル**の使用を強く推奨。**アカウント ID** を設定すると混同を防止できます。
...
同一中継 URL と API Key を使用。アカウント ID は任意（PSID 自動認識可）。

## 🖥 UI

ツールバーアイコンで **右固定サイドバー** を開きます。

- **サイドバー**：アカウント状態、マスク済み PSID、動作ログ、**今すぐ確認**、**強制更新＆送信**、**設定**
