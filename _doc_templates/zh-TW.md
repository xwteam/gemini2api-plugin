...
5. 安裝後點擊擴充功能圖示 → 在右側開啟側邊欄 → 右上角 **設定**，完成配置（見下文）
...
| **API Key** | 中轉站的 `sk-` 開頭金鑰 |
| **帳號 ID** | **建議填寫**（如 `account-0`）：填了就按該帳號直接提交，Cookie 更新後也能正常保活。留空則靠本地 PSID 自動匹配帳號，但 Cookie 變化大時可能匹配不上 |
...
> - 建議**在設定頁填寫本瀏覽器負責的帳號 ID**：填了就按帳號直接提交（Cookie 更新後也能保活），並天然避免串號；留空時靠 PSID 自動匹配，多帳號場景可能認錯或匹配不上。
...
每個環境裡的擴充功能都填同一個中轉站地址和 API Key，帳號 ID 可留空（擴充功能靠 PSID 自動認出本瀏覽器負責哪個帳號）或精確填寫。這樣多個瀏覽器各管各的帳號，互不干擾。
...
- **側邊欄面板**：顯示各帳號活動/過期狀態、PSID 打碼預覽、最近動作日誌；`立即檢查` 手動跑一輪輪詢，`強制重新整理提交` 立即重新整理並提交；右上角 `設定` 進入配置頁。
