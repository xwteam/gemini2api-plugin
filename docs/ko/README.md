...
| **API Key** | 중계 `sk-` 접두사 키 |
| **계정 ID** | **권장** (`account-0` 등): 해당 계정에 직접 제출. 비우면 PSID 자동 매칭 |
| **폴링 간격** | 기본 60초, 최소 30초 |
| **새로고침 쿨다운** | 기본 120초 |

**연결 테스트** 후 **저장** (중계 도메인 권한 허용).

> [!IMPORTANT]
> **브라우저 인스턴스당 계정 1개만** 유지 가능. 로그인된 **gemini.google.com 탭을 상시 열어두고**, **전용 브라우저 프로필** 사용 권장. **계정 ID** 설정 시 혼선 방지.
...
동일 중계 URL·API Key 사용. 계정 ID는 선택(PSID 자동 인식 가능).

## 🖥 UI

툴바 아이콘 → **오른쪽 고정 사이드바**.

- 상태·마스킹된 PSID·동작 로그·**지금 확인**·**강제 새로고침 제출**·**설정**
