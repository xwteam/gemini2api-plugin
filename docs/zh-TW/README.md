...
3. 開啟右上角的 **開發人員模式**
4. 點擊 **載入未封裝項目**，選擇本專案根目錄（含 `manifest.json` 的資料夾）

> [!IMPORTANT]
> Chrome 禁止擴充功能目錄內出現 `_` 開頭的檔案/資料夾（**`_locales` 除外**）。v1.2.2 起開發用 `doc-templates/`、`gen_docs.py` 已去掉 `_` 前綴；若仍報 `_doc_templates` 相關錯誤，請拉取最新程式碼。
...
A：請更新到 **v1.2.2+** 並重新拉取倉庫。舊版含 `_` 前綴的開發目錄會導致 Chrome 拒絕載入。
...
|------|------|
| `README.md` | 簡體中文主文檔 |
| `doc-templates/` | 繁中 / 英 / 日 / 韓 文檔模板 |
| `gen_docs.py` | 生成 `docs/{locale}/README.md` |
| `CHANGELOG.md` | 版本變更記錄 |

更新文檔：改 `README.md` 或 `doc-templates/*.md` 後執行 `python gen_docs.py` 重新生成五語 `docs/`。
